...
king_mage <- get_decennial(state = "WA", county = "King", geography = "tract", year = 2010, variables = c(median_age = median_age_both_sex), geometry = TRUE, cb = FALSE)
...
```{r}
sea_neighs_mage <- seattle_tracts %>%
  group_by(tract, cra_name) %>%
  tally() %>%
  select(-n) %>%
  right_join(king_mage, by = "tract") %>%
